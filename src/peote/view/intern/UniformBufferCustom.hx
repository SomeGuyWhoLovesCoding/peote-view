...
class UniformBufferCustom 
{
    ...
    private var _vector:Array<Float> = [0.0, 0.0, 0.0, 0.0];
    ...
    public inline function updateFloat(gl:PeoteGL, index:Int, value:Float, updateBuffer:Bool = false) {
        if (gl != null) {
    ...
    public inline function updateVector(gl:PeoteGL, index:Int, values:Array<Float>, updateBuffer:Bool = false) {
        if (gl != null) {
    ...
    public inline function getVector(index:Int):Array<Float> {
        _vector[0] = vectorOffsets[index];
        _vector[1] = vectorOffsets[index + 4];
        _vector[2] = vectorOffsets[index + 8];
        _vector[3] = vectorOffsets[index + 12];
        return _vector;
    }
    
    public function createGLBuffer(gl:PeoteGL)
    {
        // Re-seed bytes from current live values so any set_value calls
        // that happened before the GL context was ready are not lost.
        for (i in 0...floats.length)
            uniformBytes.setFloat(floatOffsets[i], floats[i].value);
