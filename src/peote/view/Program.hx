...
class Program 
{
	...
	**/
	...
	private inline function cacheExtensions():Void {
		...
		extensionCache.set("EXT_sRGB_write_control", gl.getExtension("EXT_sRGB_write_control") != null);
	...
	private inline function rebuildFragmentExtensions():Void {
		...
		else if (extensionCache.get("EXT_sRGB_write_control"))
			glShaderConfig.FRAGMENT_EXTENSIONS.push({EXTENSION:"EXT_sRGB_write_control"});
		for (ext in customFragmentExtensions)
